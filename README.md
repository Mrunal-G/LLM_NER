Realizing that finetuning and using NER approach is not suitable for PII detection at all, since the lot of the entity types need context awareness to understand the PII type. Prompting various open source models and using Langchain or something similar is going to be the way to solve this problem. I have developed a prototype as seen below using opensource LLM model from Huggingface hub with LangChainJs, ReactJs, tailwindcss and HuggingFaceJs. Curently still working on it. Instruct text-generation models give better results. OpenAI gives better results than open-source Huggingface models. Going forward, I intend to use open-source Microsoft Presidio analyzer API (which I am currently exploring) in the web app. [web app code](https://github.com/Mrunal-G/LLM_app)
![](https://github.com/Mrunal-G/LLM_NER/blob/main/images/pic2.png)

I inferenced Microsoft Presidio on demo data PII text csv and the results are better than all other approaches but still not satisfactory as seen in [here](https://github.com/Mrunal-G/LLM_NER/tree/main/Presidio).
From here on I am going to try 2 options:
- Customizing Presidio for better results
- Creating Prompt templates with LLM and using it with Presidio
