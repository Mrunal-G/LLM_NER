I realized that finetuning and using pure NER approach is not suitable for PII detection at all, since lot of the custom entity types provided need context awareness to understand the PII type. Hence, I believe that prompting various open source models and using Langchain or something similar is going to be the way to solve this problem. I have developed a prototype as seen below with LangChainJs, ReactJs, tailwindcss and HuggingFaceJs. Curently still working on it. Instruct text-generation models give better results. OpenAI gives better results than open-source Huggingface models. Going forward, I intend to use open-source Microsoft Presidio analyzer API (which I am currently exploring) in the web app. [web app code](https://github.com/Mrunal-G/LLM_app)
